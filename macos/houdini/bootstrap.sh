...
if [ "houdini" != "$system_hostname" ]; then
	log_warning ">>> This bootstrap script belongs to a another host: houdini".
...
homebrew_cli=(
	7zip aria2 asdf bat bash bash-completion@2 bear bzip2 chatgpt coreutils eza
	fd findutils fish font-jetbrains-mono-nerd-font fzf gh git-delta gnupg gsed
	jq lf libpq miniserve mkcert moreutils neovim oha openfortivpn pbzip2 pigz
...
homebrew_casks=(
	...
	docker fork iina kitty mac-mouse-fix ${microsoft[*]} numi obs rectangle
...
source "$shared_dir"/install-hosts.sh houdini
...
asdf_plugins=("bun:1*" "golang:" "nodejs:14" "nodejs:18*" "zig:")
